...
template <typename T> std::optional<T> get_optional(const json &j, const std::string &key) {
    if (j.contains(key) && !j.at(key).is_null())
        return j.at(key).get<T>();
    return std::nullopt;
}

template <typename T> T get_value(const json &j, const std::string &key, const T &defaultValue) {
    if (j.contains(key) && !j.at(key).is_null())
        return j.at(key).get<T>();
    return defaultValue;
}
