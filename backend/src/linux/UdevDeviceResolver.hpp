...
private:
    ...
    template<typename T, auto UnrefFn>
    using SdUniquePtr =
        std::unique_ptr<T, SdDeleter<T, UnrefFn>>;
...
public:
    DeviceInfo resolve(std::string_view devNode) {
        ...
        if (stat(devNode.data(), &st) < 0)
            throw ResolveInfoError(
                std::format("Could not extract deviceInfo from devnode: {}",
                            devNode));
        ...
        if (sd_device_get_parent_with_subsystem_devtype(device.get(), "usb", "usb_device", &usb) < 0) {
            sd_device* disk = nullptr;
            if (sd_device_get_parent_with_subsystem_devtype(device.get(), "block", "disk", &disk) >= 0) {
        ...
        auto setIfPresent =
            [usb](const char* attr, auto setter) {
    ...
    std::vector<std::string> getUsbDevNodes() {
        using SdDeviceEnumeratorPtr =
        SdUniquePtr<sd_device_enumerator, sd_device_enumerator_unref>;
