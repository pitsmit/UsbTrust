...
DefaultApi::DefaultApi(const std::shared_ptr<Pistache::Rest::Router> &rtr, Facade &f)
    : ApiBase(rtr), facade(f) {}
...
void DefaultApi::setupRoutes() {
    ...
    Routes::Get(
        *router, "/api/v1/whitelist", Routes::bind(&DefaultApi::get_usb_white_list_handler, this));
    Routes::Get(*router,
                "/api/v1/list",
    ...
    Routes::Post(*router,
                 "/api/test/seed/whitelist",
                 Routes::bind(&DefaultApi::seed_whitelist_handler, this));
#endif
}

void DefaultApi::add_device_to_white_list_handler(const Pistache::Rest::Request &request,
    ...
    try {
        ...
        json result = {{"id", *command.record.device_id}};
...
void DefaultApi::delete_device_from_white_list_handler(const Pistache::Rest::Request &request,
                                                       Pistache::Http::ResponseWriter response) {
    try {
...
void DefaultApi::patch_valid_to_device_handler(const Pistache::Rest::Request &request,
                                               Pistache::Http::ResponseWriter response) {
    try {
        size_t id = std::stoul(request.param(":id").as<std::string>());
        json body = json::parse(request.body());
...
void DefaultApi::get_usb_white_list_handler(const Pistache::Rest::Request &,
                                            Pistache::Http::ResponseWriter response) {
    try {
